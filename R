...
names(dati)
dataset=dati[,c("importo", "sconto_d", "eta","count","genere_M","Torino","si2014","costi_entrate","numero_persone_diverse","cintura_1","cintura_2","altrove")]
dataset=na.omit(dataset)
...
mylogit2=glm(si2014  ~ importo + sconto_d + eta+count+genere_M+Torino+cintura_1+cintura_2+numero_persone_diverse, data = trainset, family = "binomial")
testset1=testset
testset1$prob.logit=predict(mylogit2,testset,type='response')
#####valutazione accurancy su soglia 0.5####
predclass= ifelse(testset1$prob.logit > 0.5 , 1,0)
table(predclass,testset$si2014)
table(testset$si2014)
#accuracy
(table(predclass,testset$si2014)[1,1]+table(predclass,testset$si2014)[2,2])/(length(testset1$si2014))


#Roc logit
logit.rocr=prediction(testset1$prob.logit,testset1$si2014)
logit.perf=performance(logit.rocr , "tpr","fpr")

labels(testset1)
model1=testset1[order(-testset1[,13]),]


## tree ######
library("party")
testset2=testset
ctre=ctree(si2014 ~ importo + sconto_d + eta+count+genere_M+Torino+cintura_1+cintura_2+numero_persone_diverse, data=dataset[ind == 1,])

testset2$tree.prob=unlist(treeresponse(ctre,testset2))

tree.rocr=prediction(testset2$tree.prob,testset2$si2014)
tree.perf=performance(tree.rocr , "tpr","fpr")


model2=testset2[order(-testset2[,13]),]

predclass= ifelse(testset2$tree.prob > 0.5 , 1,0)
table(predclass,testset2$si2014)
#accuracy su soglia 0.5#####
(table(testset2$si2014,predclass)[1,1]+table(testset2$si2014,predclass)[2,2])/(length(testset2$si2014))
...
library("e1071")
#svm_tune=tune(svm, si2014 ~ ., data = trainset,
...
x.svm=svm(si2014~importo + sconto_d + eta+count+genere_M+Torino+cintura_1+cintura_2+numero_persone_diverse, data = trainset, cost=3, gamma=0.6,kernel="radial",probability = TRUE)
summary(x.svm)
testset5$x.svm.prob <- predict(x.svm, type="prob", newdata=testset5, probability = TRUE)
summary(testset5$x.svm.prob)
svm.rocr <- prediction(testset5$x.svm.prob,testset5$si2014)
svm.perf <- performance(svm.rocr, "tpr","fpr")

model5=testset5[order(-testset5[,13]),]
predclass= ifelse(testset5$x.svm.prob > 0.5 , 1,0)
table(predclass,testset5$si2014)
#accuracy su soglia 0.5#####
(table(predclass,testset5$si2014)[1,1]+table(predclass,testset5$si2014)[2,2])/dim(testset5)[1]

###################forest#########

testset4=testset
library("randomForest")
#bestmtry=tuneRF(trainset, trainset$si2014, stepFactor=1.5, improve=1e-5, ntree=500)

forest.model=randomForest(as.factor(si2014) ~ importo + sconto_d + eta+count+genere_M+Torino+cintura_1+cintura_2+numero_persone_diverse,  
                             data = dataset[ind == 1,], 
                             ntree=500, mtry=4,
                             type="classification")

forest.pred=predict(forest.model, newdata =testset4 , type = "prob")
testset4$forest.pred=forest.pred[,2]
predclass= ifelse(testset4$forest.pred > 0.5 , 1,0)

#accuracy su soglia 0.5#####
(table(predclass,testset4$si2014)[1,1]+table(predclass,testset4$si2014)[2,2])/dim(testset4)[1]

forest.rocr=prediction(testset4$forest.pred,testset4$si2014)
forest.perf=performance(forest.rocr, "tpr","fpr")

plot(forest.perf, col=2, main="ROC curves comparing classification performance of 3 models")

model4=testset4[order(-testset4[,13]),]
model4
...
x.rp=rpart( si2014~ importo + sconto_d + eta+count+genere_M+Torino+cintura_1+cintura_2+numero_persone_diverse, data=dataset[ind == 1,])
testset3$x.rp.prob=predict(x.rp,testset3)
predclass= ifelse(testset3$x.rp.prob > 0.5 , 1,0)
#accuracy su soglia 0.5#####
(table(predclass,testset3$si2014)[1,1]+table(predclass,testset3$si2014)[2,2])/dim(testset3)[1]

x.rp.prob.rocr=prediction(testset3$x.rp.prob,testset3$si2014)
x.rp.perf=performance(x.rp.prob.rocr, "tpr","fpr")

model3=testset3[order(-testset3[,13]),]
names(model3)
...
model=naiveBayes(si2014~ importo + sconto_d + eta+count+genere_M+Torino+cintura_1+cintura_2+numero_persone_diverse, data = dataset[ind == 1,])
testset6$naiveBayes=predict(model,testset6, type = "raw")[,2]
naive.rocr=prediction(testset6$naiveBayes,testset6$si2014)
naive.perf=performance(naive.rocr, "tpr","fpr")


#accuracy su soglia 0.5#####
table(testset6$si2014)

predclass= ifelse(testset6$naiveBayes > 0.5 , 1,0)
table(predclass,testset6$si2014)
(table(predclass,testset6$si2014)[1,1]+table(predclass,testset6$si2014)[2,2])/dim(testset6)[1]
model6=testset6[order(-testset6[,13]),]
...
plot(naive.perf, col=1,main="ROC")
#plot(svm.perf,col=2,add=T)
plot(tree.perf,col=3,add=T)
plot(forest.perf,col=4,add=T)
plot(logit.perf,col=5,add=T)
plot(x.rp.perf,col=6,add=T)
abline(a=0, b=1)



naive.d= data.frame(FPR=naive.perf@x.values[[1]],TPR=naive.perf@y.values[[1]])
#svm.d= data.frame(FPR=svm.perf@x.values[[1]],TPR=svm.perf@y.values[[1]])
tree.d= data.frame(FPR=tree.perf@x.values[[1]],TPR=tree.perf@y.values[[1]])
forest.d= data.frame(FPR=forest.perf@x.values[[1]],TPR=forest.perf@y.values[[1]])
logit.d= data.frame(FPR=logit.perf@x.values[[1]],TPR=logit.perf@y.values[[1]])
x.rp.d= data.frame(FPR=x.rp.perf@x.values[[1]],TPR=x.rp.perf@y.values[[1]])

##################ROC CURVE CON GGPLOT2

d = ggplot() +
  geom_line(data = naive.d, aes(  FPR , TPR,colour="Naive Bayes"))+
  geom_line(data = svm.d, aes(  FPR , TPR,colour="Support vector machine"))+
  geom_line(data = tree.d, aes(  FPR , TPR,colour="Conditional inference trees"))+
  geom_line(data =forest.d, aes(  FPR , TPR,colour="Random forest"))+
  geom_line(data = logit.d, aes(  FPR , TPR,colour="Logit"))+
  geom_line(data = x.rp.d, aes(  FPR , TPR,colour="Recursive partitioning"))+
  annotate("segment", x=0, xend=1,y=0, yend=1)+
  scale_color_manual(values = c(
    'Naive Bayes'='dodgerblue4','Support vector machine'='deepskyblue4',
    'Conditional inference trees'='limegreen','Random forest'='forestgreen','Logit'='red4','Recursive partitioning'='coral4'
     )) +
  labs(color = 'Models')+
  xlab("False Positive Rate (1-Specificity)")+ylab("True Positive Rate (Sensitivity)")+ggtitle("ROC Curve")+theme_wsj()+theme(plot.title = element_text(hjust = 0.5))+theme(axis.title=element_text(size=12))
d

ggplotly(d)

#######
table(dataset$si2014)/length(dataset$si2014)

############################ PROFIT CURVE ##################
#modello1 logit
#modello2 tree 
#modello3 Recursive Partitioning
#model4 forest
#model5 supervector
#model6 NAIVE BAYES 
model1$revenue=((model1$importo-(model1$costi_entrate/2))*model1$si2014)-1
model2$revenue=((model2$importo-(model2$costi_entrate/2))*model2$si2014)-1
model3$revenue=((model3$importo-(model3$costi_entrate/2))*model3$si2014)-1
model4$revenue=((model4$importo-(model4$costi_entrate/2))*model3$si2014)-1
model5$revenue=((model5$importo-(model5$costi_entrate/2))*model5$si2014)-1
model6$revenue=((model6$importo-(model6$costi_entrate/2))*model6$si2014)-1

profitti_comulati1=cumsum(model1$revenue)
profitti_comulati2=cumsum(model2$revenue)
profitti_comulati3=cumsum(model3$revenue)
profitti_comulati4=cumsum(model4$revenue)
#profitti_comulati5=cumsum(model5$revenue)
profitti_comulati6=cumsum(model6$revenue)

length(model1$revenue)
length(model2$revenue)
length(model3$revenue)
length(model4$revenue)
length(model5$revenue)
length(model6$revenue)
consumatori=seq(from=1/length(model1$importo),to=1,by=1/length(model1$importo))
data_finale=data.frame(consumatori,profitti_comulati1,profitti_comulati2,profitti_comulati3,profitti_comulati4,#profitti_comulati5,
                       profitti_comulati6)
...
profit_line=ggplot(data=data_finale,aes(x=consumatori))+
  geom_line(aes(y=profitti_comulati1,colour="Logit"))+
  geom_line(aes(y=profitti_comulati2,colour="Conditional inference trees"))+
  geom_line(aes(y=profitti_comulati3,colour="Recursive partitioning"))+
  geom_line(aes(y=profitti_comulati4,colour="Random forest"))+  
  geom_line(aes(y=profitti_comulati5,colour="svm"))+
  geom_line(aes(y=profitti_comulati6,colour="Naive Bayes"))+xlab("Percentuale di rispondenti ordinati per probabilit??")+ylab("Profitti cumulati")+labs(colour= "Modelli")+ ggtitle("Profit lines")+
geom_vline(xintercept = 0.2466943, linetype="dotted",color = "blue", size=0.5)+theme_wsj()+theme(plot.title = element_text(hjust = 0.5))+theme(axis.title=element_text(size=12))


profit_line
ggplotly(profit_line)


model1$pofitti_cumulati1=profitti_comulati1
model2$pofitti_cumulati2=profitti_comulati2
model3$pofitti_cumulati3=profitti_comulati3
model4$pofitti_cumulati4=profitti_comulati4
#model5$pofitti_cumulati5=profitti_comulati5
model6$pofitti_cumulati6=profitti_comulati6


model1[5000,15]
model2[5000,15]
model3[5000,15]
model4[5000,15]#miglior modello 4.
model6[5000,15]

############################ ISOPROFIT CON EXPETCTED VALUE###########

model1=model1[,-dim(model1)[2]]

model2=model2[,-dim(model2)[2]]
model3=model3[,-dim(model3)[2]]
model4=model4[,-dim(model4)[2]]
#model5=model5[,-dim(model5)[2]]
model6=model6[,-dim(model6)[2]]

model1$exp=((model1$importo-(model1$costi_entrate/2)))*(model1$prob.logit)-1
model2$exp=((model2$importo-(model2$costi_entrate/2)))*(model2$tree.prob)-1
model3$exp=((model3$importo-(model3$costi_entrate/2)))*(model3$x.rp.prob)-1
model4$exp=((model4$importo-(model4$costi_entrate/2)))*(model4$forest.pred)-1
#model5$exp=((model5$importo-(model5$costi_entrate/2)))*(model5$x.svm.prob)-1
model6$exp=((model6$importo-(model6$costi_entrate/2)))*(model6$naiveBayes)-1     
                                                                          

model1=model1[order(-model1$exp),]
model2=model2[order(-model2$exp),]
model3=model3[order(-model3$exp),]
model4=model4[order(-model4$exp),]
#model5=model5[order(-model5$exp),]
model6=model6[order(-model6$exp),]
...
pp=ggplot(data=data_finale,aes(consumatori))+geom_line(aes(y=profitti_comulati1,colour="Logit"))+geom_line(aes(y=profitti_comulati2,colour="Conditional inference trees"))+geom_line(aes(y=profitti_comulati3,colour="Recursive partitioning"))+geom_line(aes(y=profitti_comulati4,colour="Random forest"))+geom_line(aes(y=profitti_comulati5,colour="svm"))+
  geom_line(aes(y=profitti_comulati6,colour="Naive Bayes"))+labs(colour= "Models")+geom_vline(aes(xintercept = 0.2466943,linetype= "Budget constraint"),color="red", size=0.5)+scale_linetype_manual(name = "Constraint",values = c("Budget constraint" = "dotted"))+
xlab("Percentage of subscribers ordered by expected profits")+ylab("Cumulative profits")+ ggtitle("Profit lines")

profit_line=pp+theme_wsj()+scale_y_continuous(labels=scales::comma,breaks=seq(0,400000,by=50000),limits=c(0,300000) )+theme(plot.title = element_text(hjust = 0.5))+theme(axis.title=element_text(size=12))
...
data_finale[5000,3]
data_finale[5000,4]
data_finale[5000,5]#miglior modello 4, Random Forest.
data_finale[5000,6]
data_finale[5000,7]                                                                        
...
class(dati_completi$data_inizio)
dati_completi$data_inizio=as.Date(as.character(dati_completi$data_inizio),format="%d/%m/%Y")
dati_completi
dati_completi$data_finale=as.Date("2013-12-31")
dati_completi$diff_date=difftime(dati_completi$data_finale,dati_completi$data_inizio, units = c("days"))


dati_giorni_ribaso=dati_completi %>%
  group_by(codcliente) %>%
  select(-(data_inizio:data_finale)) %>%
  filter (! duplicated(codcliente))

dataset=merge(dati,dati_giorni_ribaso, by="codcliente",all.x=T)
labels(dataset)

dataset=dataset[,c("importo", "sconto_d", "eta","count","genere_M","Torino","si2014","costi_entrate","numero_persone_diverse","cintura_1","cintura_2","altrove","diff_date")]
dataset=na.omit(dataset)


dataset$diff_date=as.numeric(as.character(dataset$diff_date))
...
predclass= ifelse(testset1$prob.logit > 0.5 , 1,0)
table(testset$si2014,predclass)
#accuracy
(table(testset1$si2014,predclass)[1,1]+table(testset1$si2014,predclass)[2,2])/(length(testset1$si2014))

model1=testset1
labels(model1)
model1=testset1[order(-testset1[,14]),]
...
#accuracy
predclass= ifelse(testset2$tree.prob > 0.5 , 1,0)
table(testset2$si2014,predclass)
(table(testset2$si2014,predclass)[1,1]+table(testset2$si2014,predclass)[2,2])/(length(testset2$si2014))
model2=testset2[order(-testset2[,14]),]
...
testset4$forest.pred=forest.pred[,2]
predclass= ifelse(testset4$forest.pred > 0.5 , 1,0)
model4=testset4[order(-testset4[,14]),]
...
model3=testset3
model3=testset3[order(-testset3[,14]),]
...
(table(predclass,testset6$si2014)[1,1]+table(predclass,testset6$si2014)[2,2])/dim(testset6)[1]
model6=testset6
model6=testset6[order(-testset6[,14]),]

##########################################
model1$costi_entrate_ribasati=(model1$costi_entrate/model1$diff_date)*365
model2$costi_entrate_ribasati=(model2$costi_entrate/model2$diff_date)*365
model3$costi_entrate_ribasati=(model3$costi_entrate/model3$diff_date)*365
model4$costi_entrate_ribasati=(model4$costi_entrate/model4$diff_date)*365
#model5$costi_entrate_ribasati=(model5$costi_entrate/model5$diff_date)*365
model6$costi_entrate_ribasati=(model6$costi_entrate/model6$diff_date)*365


model1$exp=((model1$importo-(model1$costi_entrate_ribasati/2)))*(model1$prob.logit)-1
model2$exp=((model2$importo-(model2$costi_entrate_ribasati/2)))*(model2$tree.prob)-1
model3$exp=((model3$importo-(model3$costi_entrate_ribasati/2)))*(model3$x.rp.prob)-1
model4$exp=((model4$importo-(model4$costi_entrate_ribasati/2)))*(model4$forest.pred)-1
#model5$exp=((model5$importo-(model5$costi_entrate_ribasati/2)))*p()-1
model6$exp=((model6$importo-(model6$costi_entrate_ribasati/2)))*(model6$naiveBayes)-1     


model1$revenue=((model1$importo-(model1$costi_entrate_ribasati/2))*model1$si2014)-1
model2$revenue=((model2$importo-(model2$costi_entrate_ribasati/2))*model2$si2014)-1
model3$revenue=((model3$importo-(model3$costi_entrate_ribasati/2))*model3$si2014)-1
model4$revenue=((model4$importo-(model4$costi_entrate_ribasati/2))*model3$si2014)-1
#model5$revenue=((model5$importo-(model5$costi_entrate_ribasati/2))*model5$si2014)-1
model6$revenue=((model6$importo-(model6$costi_entrate_ribasati/2))*model6$si2014)-1
...
pp=ggplot(data=data_finale,aes(consumatori))+geom_line(aes(y=profitti_comulati1,colour="Logit"))+geom_line(aes(y=profitti_comulati2,colour="Conditional inference trees"))+geom_line(aes(y=profitti_comulati3,colour="Recursive partitioning"))+geom_line(aes(y=profitti_comulati4,colour="Random forest"))+#geom_line(aes(y=profitti_comulati5,colour="svm"))+
  geom_line(aes(y=profitti_comulati6,colour="Naive Bayes"))+labs(colour= "Models")+geom_vline(aes(xintercept = 0.2466943,linetype= "Budget constraint"),color="red", size=0.5)+scale_linetype_manual(name = "Constraint",values = c("Budget constraint" = "dotted"))+
  xlab("Percentage of respondents with greater probability")+ylab("Cumulative profit")+ ggtitle("Profit lines with annualized costs")

profit_line=pp+theme_wsj()+scale_y_continuous(labels=scales::comma,breaks=seq(0,400000,by=50000),limits=c(0,300000) )+theme(plot.title = element_text(hjust = 0.5))
